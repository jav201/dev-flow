# Architecture — module map — <PROJECT>
...
> **No reserved-field block.** What a rule reads out of this artifact is `V8`'s `` `path/**` `` **globs** — paths, not words — so there is no label here for a translation to lose (flow rev72, `T09`; the population and its reasons live in `LANG RESERVED-population-closed`).
